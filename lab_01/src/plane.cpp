...
void Plane::drawAnswer(QPainter &painter)
{
    ...
    if (std::isinf(y))
        painter.drawLine(realCoordToScreenCoord(center), realCoordToScreenCoord(trianglePoint));
}
...
void Plane::scale()
{
    ...
    calcScaleFactorByDelta(deltaX, deltaY, minRatioNoScale, scaleAmount);
}
...
void Plane::scaleByAnswer()
{
    ...
    calcScaleFactorByDelta(deltaX, deltaY, 0.9, 0.9);
...
void Plane::calcScaleFactorByDelta(double deltaX, double deltaY, double threshold, double amount)
{
    const int w = this->viewport()->width();
    const int h = this->viewport()->height();

    if (deltaX / w < threshold || deltaY / h < threshold)
        scaleFactor = std::min(w * amount / deltaX, h * amount / deltaY);
